...
- distinct returns number of unique values in a numeric or string vector. I no longer use this function since discovering that data.table has uniqueN() which does the same thing.
- ipolate(x,y,extrapolate=FALSE): interpolates to replace missing data based on a linear function of the observed data and a variable (usually time). ipolate(x,y) used to only interpolate, nows an option extrapolate=TRUE.  this will carry forward the last non-missing observation to replace subsequent NA. And it carries backward non-missing observations to replace preceding missings. ipolate() behaves this way because it is based on an R function called approx() that does it this way. ipolate() is different from the data.table nafill() function which does either locf or nocb but does not do linear interpolation between observed values.
- ipolate_stata(x,y,epolate=FALSE). for linear extrapolation as an option in addition to linear interpolation,  a new function that has been tested and compared with Stata's ipolate command. the only important difference is Stata ipolate expects the yvar before the xvar to 
...
- str2tex_col() is a replacement for texout(). It is more flexible (can specify different rounding digits for different columns) and eliminates some bugs.
